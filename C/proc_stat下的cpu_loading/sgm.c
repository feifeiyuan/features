...
const u8 min_core, const u8 max_core, float  *pcpu_per, ulli cluster_count, ulli total_count)
{	
...
si processor_num, ulli cluster_little, ulli cluster_big, ulli total_count)
{
	...
	LIT_CLUSTER_START, LIT_CLUSTER_END, pcpu_per, cluster_little,  total_count);
	
	if(processor_num > LIT_CLUSTER_END){
		ret_total += print_result(cpu4_size, cpu4_available_freq, 
		LIT_CLUSTER_END, BIG_CLUSTER_END, pcpu_per, cluster_big,  total_count);
		printf("SUM : %lf\n", ret_total);
	}
