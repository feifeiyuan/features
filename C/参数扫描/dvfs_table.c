...
s8 get_init_freq_count(const char *cpu_cur_path, u8 *size, ui *cpu_available_freq)  
{  
    FILE    *fp = NULL;  
...
int write_node(int value, char *path)
{
	FILE *fp  = NULL;
	fp = fopen(path, "w");
	if(fp==NULL){
...
int switch_governor(char *path, char *governor)
{
	FILE *fp  = NULL;
	fp = fopen(path, "w");
	if(fp==NULL){
...
int fix_freq_get_vdd(int cpu_size, int *cpu_available_freq, char *path, char *governor_path)
{
	int i = 0;
